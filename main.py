...
st.set_page_config(page_title="YZF Scraping", page_icon=":mag:", layout="wide")
...
startups = {
    ...
    "Teknoloji ve Yazılım (Technology and Software)": ["Adapha", "Adin.ai", "Aivisiontech", "Co-One", "Cuebric", "Faradai AI", "Khenda", "Syntonym", "Lumion", "Phitech"],
    ...
    "Diğer (Other)": ["Adastec", "Crait.it", "Lifespin GMBH", "Madlen", "Rem People", "Tazi"]
}


prioritized_sites = ['webrazzi.com', 'tomorrow.com.tr', 'swipeline.co', 'egirisim.com', 'girisimhaber', 'startupwatch']


def google_search(query, api_key, search_engine_id, start_date=None, end_date=None, num_results=100):
    ...
    while len(results) < num_results:
        ...
        if response.status_code == 200:
            ...
            for item in items:
                ...
                item['date'] = date or item.get('pagemap', {}).get('metatags', [{}])[0].get('article:published_time', '')
...
def suggest_corrections(keyword, results):
    ...
    most_common_keywords = [word for word, count in keyword_counts.most_common() if word.lower() != keyword.lower()]
...
def main():
    ...
    st.info("Bu uygulama belirli Startup ve anahtar kelimeler için Google arama sonuçlarını gösterir ve analiz eder.")
    st.info("Bu uygulama ile sektör ve şirket özelinde Google arama sonuçlarını görüntüleyebilirsiniz.")
    ...
    start_date = st.sidebar.date_input("Başlangıç Tarihi", datetime.today() - timedelta(days=1))
    end_date = st.sidebar.date_input("Bitiş Tarihi", datetime.today())

    if st.button("Ara"):
        results = google_search(keyword, API_KEY, SEARCH_ENGINE_ID, start_date.strftime('%Y%m%d'), end_date.strftime('%Y%m%d'))
        if results:
